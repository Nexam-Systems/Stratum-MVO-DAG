...
def find_repo_root(start_path: Path | None = None) -> Path:
    """
    Find the QGC source root.

    STRATUM: QGC is vendored inside a larger monorepo whose .git lives one level
    above this tree, so anchoring purely on ".git" walks past the QGC root and
    resolves tools/.venv/pyproject paths in the wrong directory. We anchor on an
    explicit ".qgc-root" marker first (present only at the QGC tree root), then
    fall back to ".git" for a standalone QGC checkout.
    ...
    # Prefer the explicit QGC-root marker over .git.
    for parent in [current, *current.parents]:
        if (parent / ".qgc-root").exists():
            return parent
